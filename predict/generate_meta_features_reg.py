...
def get_gain_importance(data, label, n_jobs):
    gbm = lgb.LGBMRegressor(**{'importance_type': 'gain', 'seed': 0, 'deterministic': True,
...
def get_shap_score(data, label, n_jobs):
    ...
    shap_values = np.mean(np.abs(explainer.shap_values(data.values)), 0)
...
def run_calculation_simple(data, label, numerical_features, categorical_features, logger, n_jobs=8):
    ...
    anova_p_value, welch_anova_p_value = get_anova_p_value_reg(data.columns, data_ray, label_ray)
...
def get_meta_features_from_csv_reg(data_x, data_y, numerical_features, categorical_features, logger, n_jobs=-1):
