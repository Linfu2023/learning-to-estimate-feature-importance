...
def train_and_predict(data_train, data_val, label_version, seed):
    try:
        ...
        with open(os.path.join(directory, f"data/train_data/d_train_{label_version}_clf.pkl"), "wb") as f:
            pickle.dump(d_train, f)
        with open(os.path.join(directory, f"data/valid_data/d_val_{label_version}_clf.pkl"), "wb") as f:
            pickle.dump(d_val, f)

        d_train = pickle.load(open(os.path.join(directory, f"data/train_data/d_train_{label_version}_clf.pkl"), 'rb'))
        d_val = pickle.load(open(os.path.join(directory, f"data/valid_data/d_val_{label_version}_clf.pkl"), 'rb'))
...
def run(label_version, seed):
    try:
        ...
        data_train = pd.read_csv(os.path.join(directory, 'data/train_data/meta_features_LTE_train_v%d_clf.csv' % label_version))
        data_val = pd.read_csv(os.path.join(directory, 'data/valid_data/meta_features_LTE_valid_v%d_clf.csv' % label_version))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", "--output_dir", help="output directory of models", type=str, default="models/temp_LTE_models_clf")
